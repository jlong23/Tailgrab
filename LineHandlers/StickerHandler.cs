...
public class StickerHandler : AbstractLineHandler
{
    ...
    public static readonly string LOG_PATTERN = @"([\d]{4}.[\d]{2}.[\d]{2}\W[\d]{2}:[\d]{2}:[\d]{2})\W(Log[\W]{8}|Debug[\W]{6})-\W\W(?:[\[Always\]\W]*)\[StickersManager\]\WUser\W(usr_[\d\w\W]+)\W\(([\d\w\W]+)\)\Wspawned\Wsticker\W(file_[\d\w\W]+)";
    ...
    public override bool HandleLine(string line)
    {
        ...
        if( m.Success )
        {
            ...
            if( LogOutput )
            {
                logger.Info($"{COLOR_PREFIX}{userName} ({userId}) - {fileURL}{COLOR_RESET}");
            }
            PlayerManager.AddPlayerEventByDisplayName(userName, PlayerEvent.EventType.Sticker, $"Spawned sticker: {fileURL}");
